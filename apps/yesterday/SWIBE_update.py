...
# Load the table into a Pandas DataFrame
    try:
        ...
        df = pd.read_sql_query("SELECT MAX(date) as maxDate FROM SWIBE_v", conn, parse_dates=["maxDate"])
        ...
        if getPowerMeasurements(strStartDate, strEndDate):
            ...
            result['power_Wp'] = 6.6
            result['date'] = result['date'].apply(UTC_to_str)
            result.to_sql('SWIBE_v', conn, if_exists='append', index=False )
            
            # Commit changes and close connection
            conn.commit()
            return None
        else:
            return "No data returned from SWIBE"
